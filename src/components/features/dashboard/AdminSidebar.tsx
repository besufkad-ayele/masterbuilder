    ...
    >
      <div className={cn("shrink-0 border-b border-border transition-all duration-300", isSidebarOpen ? "p-6" : "px-3 py-6")}>
        ...
        >
          ...
          >
            <p className="truncate text-lg font-semibold leading-tight text-foreground">Admin Console</p>
            <p className="truncate text-xs leading-tight text-muted-foreground">
              {currentUser?.title || "Leadership Development"}
            </p>
          </div>
        </Link>
      </div>
      ...
      <div className={cn("shrink-0 border-t border-border transition-all duration-300", isSidebarOpen ? "p-6" : "p-3")}>
        <div className={cn("rounded-xl bg-muted/70 transition-all duration-300", isSidebarOpen ? "p-4" : "p-2")}>
          {isSidebarOpen ? (
            <>
              <p className="mb-1 text-xs font-medium text-foreground">System Status</p>
...
export default function AdminSidebar({ children }: AdminSidebarProps) {
  ...
  const sidebar = <AdminSidebarPanel isSidebarOpen={isSidebarOpen} onSignOut={handleSignOut} />;

  return (
    <ResponsiveDashboardShell
      mobileTitle="Admin Console"
      sidebar={sidebar}
