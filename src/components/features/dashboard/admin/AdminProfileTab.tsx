...
export default function AdminProfileTab() {
    ...
    try {
      ...
      if (profile) {
        ...
        setUser({
          ...
          location: (profile.location as string | undefined) || undefined,
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user?.profileId || !user.user_id) return;

    setIsUpdating(true);
    try {
      if (user.role === "ADMIN") {
        await AdminManagementService.updateAdminProfile(user.profileId, user.user_id, {
          name: editData.name,
        ...
        await FacilitatorService.updateFacilitator(user.profileId, user.user_id, {
    ...
    <div className="mx-auto max-w-5xl space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        ...
        <Button
          type="button"
          onClick={openEdit}
          className="mt-3 h-10 shrink-0 gap-2 self-start sm:mt-0"
        >
        ...
        <section className="space-y-4 rounded-2xl border border-border bg-card p-5 sm:p-6 lg:col-span-3">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <Field
              icon={Shield}
              label="Title"
              value={user.title || user.role}
            />
            {joinedLabel && (
              <Field icon={Calendar} label="Member since" value={joinedLabel} />
            )}
          </div>
